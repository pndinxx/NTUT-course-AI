...
def agent_cleaner(course_name, raw_data):
    ...
    你的任務是**過濾雜訊**，只保留有關「北科大」或「教學」有關的資料。
    
    【執行規則】：
    1. **完整保留原文**：請找出資料中與「{course_name}」相關的評論、貼文或介紹，**一字不改**地保留下來。不要進行總結、不要條列式摘要。
    2. **保留連結**：每一則資料的來源網址 (URL) 必須保留，不得刪除。
    3. **刪除雜訊**：僅刪除與課程或教師無關的內容（例如：網站導航列、廣告代碼、無關的其他文章標題、亂碼）(若是有衝突的教師角色，請將方向不同的角色資料刪除)。
    4. **呈現方式**：請將清理後的資料整理成易讀的區塊，每則資料包含「來源連結」與「原始內文」。

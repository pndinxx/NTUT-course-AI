...
def agent_cleaner(course_name, raw_data):
    ...
    【執行規則】：
    1. **完整保留原文**：請找出資料中與「{course_name}」相關資訊，將符合目標的網址內容**一字不改**地將該則保留下來。不要進行總結、不要條列式摘要。
    2. **刪除雜訊**：刪除錯誤、與目標不相符、沒有北科大或教師相關的網站，例如：網站導航列、廣告代碼、無關的其他文章標題、亂碼）。
    3. **呈現方式**：資料維持原本格式不要動，每則資料包含「來源連結」與「原始內文」。
